...
const renderXtraInfo = (num, sen) => {
  $.ajax({
    ...
    beforeSend: function(xhr) {
      ...
      let committees = data.results[0].roles[0].committees;
      if (committees.length) {
        let commsStr = '<div class="top-text"><strong>Committee Memberships:</strong></div>';
        committees.forEach(
          (committee) => {
            commsStr += `<div>${committee.name}</div>`;
          }
        );
        $(`.xtra-info-${num}`).empty().append(`<div>${commsStr}</div>`);
      }
    }
  });
  $.ajax({
